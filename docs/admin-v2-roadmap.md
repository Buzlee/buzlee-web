...
1. **Flyer review detail (web)** — mobile `flyer-review/[id]` is a full detail screen (lineup, location, business, take-down). Web has only the Flyers table with a take-down menu; `useAdminFlyer` is already ported. Add `/admin/flyers/review?id=`.
2. **Create / edit flyer (web)** — mobile `create-flyer/[businessId]` + `edit-flyer/[id]` wrap `features/flyer-wizard` (~76 files). Port the wizard model/API (`flyer-wizard/model/*`, `entities/flyer` mutations, flyer media upload) and build a web form; the address picker + image field from business-form are reusable.
3. **Batch upload (web)** — mobile `features/admin-batch-upload` (CSV parse → `resolve-rows` → row edit → insert; remote media fetch). Port `lib/` + `model/` verbatim, build a web table UI. Sidebar TOOLS link currently disabled.
4. **Map + discovery feed (web)** — mobile `discovery-map` / `discovery-feed` are read-only resident views (MapLibre + MapTiler, `features/discovery`, `features/filter`). Needs `MAPTILER_API_KEY` in Vercel (dev+preview) and a web map lib. Sidebar TOOLS link currently disabled.
5. **Residents CSV export (web)** — mobile `entities/admin/lib/residents-csv.ts` + share sheet. Port the lib, add an Export button on `/admin/residents`.
6. **Live-data smoke test** on preview: approve → reject → soft delete → restore → claim approve/decline → create business with logo/cover + address → edit-details (replace/remove images, change address) → confirm map pin + images in the mobile app, and that `app.buzlee.com/admin` still 404s. Then OTA-publish the mobile claims screen to dev + preview.
7. **Dev tools** — mobile `dev-tools` is development-build only; not a parity target.
